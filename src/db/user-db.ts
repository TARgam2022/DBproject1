...
function rowToOrder(row: OrderRow, items: Order["items"]): Order {
  return {
    id: Number(row.id),
    orderId: row.orderId,
...
function rowToOrderItem(row: OrderItemRow): Order["items"][0] {
  return {
...
export class UserDB extends BaseDB {
  ...
  private async migrateOrderItems(): Promise<void> {
    if (await this.columnExists("orders", "items")) {
      const rows = await this.all<{
        id: number;
        items: string;
      }>("SELECT id, items FROM orders");
      for (const row of rows) {
        const items = parseJsonArray<Order["items"][0]>(row.items);
        for (const it of items) {
          await this.run(
            `INSERT INTO order_items
              (orderId, productId, title, price, discountedPrice, quantity)
             VALUES (?, ?, ?, ?, ?, ?)`,
            [row.id, it.id, it.title, it.price, it.discountedPrice, it.quantity]
          );
        }
      }
      await this.ddl("ALTER TABLE orders DROP COLUMN items");
    }
  }
  ...
  async createOrder(input: {
    ...
    await this.init();
    const result = await this.run(
      `INSERT INTO orders (orderId, userId, status, total)
       VALUES (?, ?, ?, ?)`,
      [
        input.orderId,
        input.userId,
    ...
    for (const it of input.items) {
      await this.run(
        `INSERT INTO order_items
          (orderId, productId, title, price, discountedPrice, quantity)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [orderId, it.id, it.title, it.price, it.discountedPrice, it.quantity]
      );
    }
    ...
    return rowToOrder(rows[0], input.items);
  }
  ...
  async getOrdersByUser(userId: number): Promise<Order[]> {
    ...
    const orders: Order[] = [];
    for (const row of rows) {
      const itemRows = await this.all<OrderItemRow>(
        "SELECT * FROM order_items WHERE orderId = ?",
        [row.id]
      );
      orders.push(rowToOrder(row, itemRows.map(rowToOrderItem)));
    }
    return orders;
  }
}
