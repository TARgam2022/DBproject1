...
export async function PUT(
  ...
  try {
    ...
    const { title, price, reviews, discountedPrice, thumbnail, preview, preview2, categoryId, categoryIds, description } = body ?? {};
    ...
    if (preview2 !== undefined) updates.preview2 = preview2;
    if (description !== undefined) updates.description = description;
    if (Array.isArray(categoryIds)) {
      updates.categoryIds = categoryIds.filter((v: unknown): v is number => typeof v === "number" && v > 0);
