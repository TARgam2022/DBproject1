...
export async function POST(request: NextRequest) {
  try {
    ...
    const { title, price, reviews, discountedPrice, thumbnail, preview, preview2, categoryId, categoryIds, description } =
      body ?? {};
    ...
    const catIds = Array.isArray(categoryIds)
      ? categoryIds.filter((v: unknown): v is number => typeof v === "number" && v > 0)
      : categoryId != null
        ? [Number(categoryId)]
        : [];
    ...
    const product = await productDB.add({
      ...
      preview2: preview2 ?? preview ?? null,
      categoryIds: catIds,
